...
public class FHIRConverters {
    ...
    {
        String theDate= v2Date;
        String fhirDate = "";

        if (v2Date == null) return fhirDate;
        if (v2Date.length() < 8) return fhirDate;
        if (v2Date.length() > 8) theDate = v2Date.substring(0,8);

        {
            String year = theDate.substring(0,4);
            String month = theDate.substring(4,6);
            String day = theDate.substring(6,8);
